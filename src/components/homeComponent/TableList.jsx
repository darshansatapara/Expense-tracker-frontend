...
// components/homeComponent/TableList.jsx
...
const TableList = ({
  ...
  // Update data based on active button
  useEffect(() => {
    setShowData(activeButton === "Today" ? today : yesterday);
  }, [activeButton, today, yesterday]);

  // Memoize formatted transactions
  const formattedTransactions = useMemo(() => {
    const data = showData?.data || [];
    return data.flatMap((expenseGroup) =>
      [...(expenseGroup.online || []), ...(expenseGroup.offline || [])].map(
        (transaction) => ({
          ...transaction,
          key: transaction._id || Math.random().toString(),
          amount: `${
            transaction.currency?.symbol || defaultCurrencySymbol
          }${parseFloat(transaction.convertedAmount || 0).toFixed(2)}`,
          date: transaction.date || "N/A",
          category: transaction.category?.name || "N/A",
          subcategory: transaction.subcategory?.name || "N/A",
          mode: transaction.mode || "N/A",
        })
      )
    );
  }, [showData, defaultCurrencySymbol]);
          ...
          <div className="hidden sm:hidden md:block lg:block">
            <div className="flex gap-5">
              ...
              <h3 className="text-md font-bold text-gray-700">
                ...
                <span className="text-green-500 font-semibold">
                  {defaultCurrencySymbol}
                  {(total?.offlineTotal || 0).toFixed(2)}
                </span>
              </h3>
            </div>
          </div>
